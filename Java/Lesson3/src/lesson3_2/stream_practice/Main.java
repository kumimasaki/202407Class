...
public class Main {
	public static void main(String[] args) {
		...
		Product mostExpensive = products.stream()
				...
				// max() の戻り値は Optional<Product> 型である
				// Optional は「値があるかもしれないし、ないかもしれない」ことを表す特別な箱のようなもの
				// .orElse(null)はOptional の中に値があればそれを返すもしくはなければ（中身が空なら）ここで指定した null を返す処理
				.orElse(null);
		...
		int totalPrice = products.stream()
				// stream() の要素は Product オブジェクト
				// sum() は 数字の合計を計算するメソッドのため、数字のストリームで呼び出す必要がある
				// Stream<Product> → IntStream （数字のストリーム）に変換 
